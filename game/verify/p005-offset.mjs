// Quick supplemental capture: offset brick-wall hit, showing the far (untouched) side of the wall.
...
async function main() {
	...
	const browser = spawn(BROWSER, [
		...
		'--window-size=1280,800', '--force-device-scale-factor=1', '--user-data-dir=/tmp/game-verify-p005-offset-brave-profile', 'about:blank',
	], { stdio: 'ignore' });
	let exitCode = 0;
	try {
		...
		// offsetM=0.9: car's centerline shifted 0.9m off the wall's center, so the impact lands well to
		// one side, leaving the OTHER end of the wall (>2m from the impact point) clear.
		await evalExpr(`window.__LAB__.setFreeConfig({ speedKmh: 50, offsetM: 0.9, angleDeg: 0 }); 'ok'`);
		...
		await evalExpr('window.__LAB__.stepN(180); "ok"');
		await evalExpr(`window.__LAB__.setCameraPreset('top'); 'ok'`);
		await sleep(500);
		await shot('brick-04-offset-top.png');
		await evalExpr(`window.__LAB__.setOrbitView({ radius: 10, height: 3.5, targetHeight: 0.8 }); window.__LAB__.setFixedAngle(-0.4); 'ok'`);
		await sleep(500);
		await shot('brick-04-offset-3q.png');
