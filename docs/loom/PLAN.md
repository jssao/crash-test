...
| G2 | Vehicle: chassis+wheel joints+powertrain+steering (design fable, impl sonnet) | fable+sonnet | **done** | 621,106 | 5/5 sim tests pass (78.9m/5s, 116.7km/h, brake 24.2m, roll 0.1°); browser drive check 0 errors; commit 27b7a34. Binding gaps found: allowFastRotation not wired (wheel ω clamp ~47rad/s → bounded engine-assist workaround), no SetMassData (ballast-sensor COM workaround), no suspension-length getter. → BINDFIX phase added |
| BINDFIX | Wire allowFastRotation + mass-data fns into shim; remove engine-assist hack; re-run all tests | sonnet | **done** | 477,636 | allowFastRotation verified (200 vs 47.12 rad/s clamp); Get/SetMassData + GetLocalCenter wired; suspension getter ABSENT upstream; assist removed → honest 91.4km/h w/ new traction taper + yaw damping; all suites green; commit c2ae3f4 |
| G3 | Damage: plastic crumple + loosening/breaking welds + detachable wheels (design fable, impl sonnet) | fable+sonnet | **done** | 594,383 | 11/11 tests (5 drive + 6 damage) incl. determinism; browser crash: crumpled hood + torn door + debris, 0 errors; weld hertz-0=rigid; loosen via new shim weld setters (commits 44c3d42, 56d590a). WATCH: FIXED_SUBSTEPS 4→12 retune (weld/traction interaction, empirical) → perf-gate risk, G4+G5 adjudicates; R-repair scope cut → assigned to G4+G5; wheel detach couples 2 wheels (accepted) |
| G4+G5 | World + polish (merged for budget): destructibles, ramps, HUD, camera polish, quality presets | sonnet | **done** | 460,440 | 131 destructibles; perf bench avg 0.140ms / p95 0.482ms (8ms gate: PASS w/ substeps 12); 11 files/13 tests green; full R-repair, 0 leaks over 48 resets (+fixed real panel-shape leak + IBL rebake bug); 4 world screenshots 0 errors; commit 79449c0. WATCH: one non-reproducible wasm OOB seen pre-fix, never recurred — playtest soak watches. DESIGN was: flat airfield (skip heightfield — untested binding path, flat fits HDRI). Destructibles all start ASLEEP, ~120-180 bodies: 3 stacked-block walls (≈6×3, varied mass), 8-high crate tower, 10-barrel bowling triangle (12-gon prism hulls), 5 tippable poles, 2 static ramps (kicker + wide). PBR via procedural CanvasTextures (crate/concrete/barrel), realistic not cartoon. Shift+R world reset. HUD: km/h, gear, rpm bar, 5-panel damage indicator fed by damage events, controls overlay, loading progress, F fps toggle. Camera: crash shake (severity-scaled), speed FOV, ground clamp. Quality presets Q cycle (shadow res/pixelRatio/AA). Perf: node bench script — physics step avg <8ms with all destructibles awake mid-crash (item 12). Audio stays stubbed (v1.1) |
...
| PLAYTEST | Scenario battery + soak via CDP (user-authorized simulations) | sonnet | in-flight | — | ramp airtime + tab-visibility untested to date; watch for one-time wasm OOB recurrence |
