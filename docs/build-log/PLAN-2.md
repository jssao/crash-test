...
| B-fix | joint.destroy(wakeAttached=false) wasm OOB trap: binding-level root-cause + fix (structures.ts reset trap, 100% repro at spawn — sleeping-bodies hypothesis) | sonnet | dispatched | | node sim reset passes (bodies awake post-crash); browser traps at spawn (all 216 asleep) → suspected vendor invariant: destroying joint between sleeping bodies w/o wake corrupts solver-set bookkeeping. Fix must land in shim (safe destroy) + regression test, not just the one call site. |
