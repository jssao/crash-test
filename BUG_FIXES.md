...
| R004 | High | Done | Hitting anything doesn't scratch, chip, or mess with paint or create things such as hanging bumpers | Screenshots: `screenshots/R004_no-paint-damage-hanging-parts/sim/` (in-sim). Reference: `screenshots/R004_no-paint-damage-hanging-parts/reference/`. |
...
- **2026-07-15 Round 4 (user-commissioned, post-RUN-6): 20/20.** P001→Done: real footwell shelf — grounds get `GROUND_CATEGORY_BITS` (`~OCCUPANT_CATEGORY_BIT`) so an occupant-only shelf can't beach the car (the old negative result is RESOLVED in vehicle/geometry.ts); shelf is speed-gated (engaged at rest, off >3.5 m/s) so every crash/ejection/flee calibration is untouched; shin-capsule BOTTOM now settles at the 0.03 floor line (was −0.14/−0.16), asserted in occupants-load-pose. R004→Done: scuff decals now legible (256px exposed-metal textures, 0.24–0.42 m, depthTest-off overlay, 2–4 per impact — eyes-on verified on the black nose) + hanging bumpers via load-time BodyShell split (scene/bumperSplit.ts; front/rear partitions registered as separate deformables, sync error ~5e-6 m; detach at ≥0.5 m front / ≥0.42 m rear plastic crush with damped droop; pristine split line invisible). Combined regression: tsc 0, sim 306/306, crash-lab 21/21.
